...
class TestBuildTrainingSession(unittest.TestCase):
    def test_get_session_val(self):
        output = get_session_val("tests/data/bsGSV/families.json", ["Tactical Defense & Offence", "SSG 6vs6+12F", "Split: Offense, Finishing Competition"], "Total Distance")
    ...
    def test_build_drills_combination(self):
        ...
        output = build_drills_combination(families_dict, "lsg", "rondo", "Total Distance")
    ...
    def test_build_session(self):
        ...
        self.assertEqual(len(output), len(ref_file), "output and reference doesn't match.")
        self.assertEqual(ref_file.keys(), output.keys(), "output and reference doesn't match.")
