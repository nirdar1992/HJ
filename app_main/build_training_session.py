...
def rec_sum(families, drills, val, fam_name, parameter, sd, sub_drills=[]):
    ...
    # if there is no more drills to check - return an empty list
        ...
        temp_val = (
            val - families[fam_name][drills[0]]["parameters"][parameter]["team's mean"]
        )
        ...
        return rec_sum(
            families, temp_drills, val, fam_name, parameter, sd, sub_drills
        ), rec_sum(families, temp_drills, temp_val, fam_name, parameter, sd, new_sbd)


def find_equal_drill(
    drill_name, sd, families_file="../data/families.json", parameter="Total Distance"
):
    """
    ...
    mean_to_reach = families[fam_name][drill_name]["parameters"][parameter][
        "team's mean"
    ]
...
def get_session_val(
    drills, families_file="../data/families.json", parameter="Total Distance"
):
    """
    :param families_file: frills families file path
    ...
    for drill in drills:
        ...
        session_dict["session's drills"][drill] = families[drill_fam][drill][
            "parameters"
        ]
...
def build_session(sb, families_file="../data/families.json"):
    ...
    for drill_index in range(len(drill_fams) - 1):
        ...
        if drill_index == 0:
            combos = build_drills_combination(
                families,
                drill_fams[drill_index],
                drill_fams[drill_index + 1],
                parameter,
            )
        # we used the second drill in the first iteration
        elif drill_index != 1:
            combos = build_drills_combination(
                families, drill_fams[drill_index], combos, parameter
            )
    ...
    for drill in last_fam:
        ...
        for combo in combos:
            ...
            # if it's in the right range - update the session options dict
                ...
                if len(drill_fams) != 2:
                    session_options[session_title]["drills"][drill] = families[
                        extract_family_name(drill)
                    ].get(drill)
                    total_variance = families[extract_family_name(drill)][drill][
                        "parameters"
                    ][parameter].get("team's variance")
                else:
                    total_variance = 0
                for dr in combo:
                    if not tuple == type(dr):
                        break
                    session_options[session_title]["drills"][dr[0]] = families[
                        extract_family_name(dr[0])
                    ].get(dr[0])
                    total_variance += families[extract_family_name(dr[0])][dr[0]][
                        "parameters"
                    ][parameter].get("team's variance")
                session_options[session_title]["total mean"] = round(total_mean, 3)
                session_options[session_title]["total variance"] = round(
                    total_variance, 3
                )
                ...
                session_options[session_title]["exceeded limit probabilities"] = (
                    round(proba_low, 3),
                    round(proba_high, 3),
                )
