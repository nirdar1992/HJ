    ...
    def check_session_params(self):
        ...
        for fam in self.session_fams:
            ...
            if fam not in families:
                raise ValueError(
                    fam + " is not in families json file, Please check your spelling."
                )
    ...
    def check_constrain_params(self):
        ...
        for con in self.constrains_dict:
            if type(self.constrains_dict[con]["fams"]) != list:
                raise ValueError(
                    "In " + con + ": families input should be a list of families."
                )
            ...
            if self.constrains_dict[con]["parameter"] not in self.valid_parameters:
                raise ValueError(
                    ...
                    + con
                    + ": "
                    + self.constrains_dict[con]["parameter"]
                    + " is not a valid parameter, please check your spelling."
                )
            if self.constrains_dict[con]["operator"] not in ["range", "sum range"]:
                raise ValueError(
                    "In " + con + ": operator must be 'range' or 'sum range'."
                )
            if (
                self.constrains_dict[con]["min_val"] < 0
                or self.constrains_dict[con]["max_val"] < 0
            ):
                raise ValueError(
                    "In " + con + ": minimum/maximum value can not be negative."
                )
            if (
                self.constrains_dict[con]["min_val"]
                > self.constrains_dict[con]["max_val"]
            ):
                raise ValueError(
                    "In " + con + ": minimum val can not be greater than maximum val."
                )
        ...
        for con in self.constrains_dict:
            ...
            for drill in drills_dict:
                ...
                if operator == "range":
                    if (
                        not min_val
                        <= drills_dict[drill]["parameters"][parameter]["team's mean"]
                        <= max_val
                    ):
                        return False
                else:  # sum range and check it at the end
                    total_sum += drills_dict[drill]["parameters"][parameter][
                        "team's mean"
                    ]
